...
querygrain(setEvidence(junction, nodes =c("starch", "sodium", "retinol", "vitamin_B2"), states = c("4","4","1","1")),nodes = c("cancer"))

# high sodium low beta-carotene check dimension 4
querygrain(setEvidence(junction, nodes =c("sodium","beta_carotene_equivalents"), states = c("4","1")),nodes = c("cancer"))

# check dimension 2, high fiber and soluble carbs and low monosaturated fatty acids and vitamin E
querygrain(
  setEvidence(
    junction,
    nodes =c("total_fiber","soluble_carbohydrates","monosaturated_fatty_acids","vitamin_E"),
    states = c("4","4","1","1")),
  nodes = c("cancer")
)
